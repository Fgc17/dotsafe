...
```bash
fatima dev --port 3333
```

This will spin up an endpoint at `http://localhost:3333/fatima` accepting any HTTP method to trigger a reload.
...
After settin up the tunneling you just need to set the webhook URL in your secret manager to the endpoint URL.
...
In this case, you can use the `fatima send-reload` command to trigger a reload.
